...
export const serverRunnerPrompt: ToolPromptSchema = {
	name: 'server_runner',
	description:
		'Start and manage server/development server instances. Used for running web applications, backend servers, or any server type locally. Supports start, stop, restart, and log retrieval. This is the designated tool for server processes - do not use git_bash or terminal tools for long-running servers. Each server instance requires a unique name for tracking.',
	parameters: {
		commandToRun: {
			type: 'string',
