...
const Tabbar = React.forwardRef<HTMLUListElement, TabbarProps>(
	({ className, onFileTypeSelect, onCameraClick, cameraDisabled, onPartnerClick, onFilesClick, ...props }, ref) => {
		...
		return (
			...
			>
				{/* 1. 👤 用户图标 (Partner) */}
				<li title="Partner" onClick={onPartnerClick} style={{ cursor: onPartnerClick ? 'pointer' : undefined }}>
				...
				<li title="Add File">
					<div className={cn("fileAdd", isOpen && "open")} onClick={handleToggle}>
						<ul>
							...
							<li className="powerpoint" onClick={(e) => { e.stopPropagation(); handleFileSelect('powerpoint') }}></li>
							<li className="excel" onClick={(e) => { e.stopPropagation(); handleFileSelect('excel') }}></li>
						</ul>
						<div>
							<span></span>
						</div>
...
// SVG Symbol Definitions Component
export const TabbarSvgSymbols: React.FC = () => (
