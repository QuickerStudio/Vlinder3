...
const GradientCard = React.forwardRef<HTMLDivElement, GradientCardProps>(
	...
	React.useEffect(() => {
		const calculateGradientScale = () => {
			...
			// 约束条件（不等式）：旋转 -90deg 后
			// 新宽度 = gradientHeight × scaleX >= cardHeight + targetExtraWidth
			// 新高度 = gradientWidth × scaleY >= cardWidth + targetExtraHeight
			
			// 求解不等式，添加小余量 0.02 防止浮点数精度问题
			const scaleX = (cardHeight + targetExtraWidth) / gradientHeight + 0.02
			const scaleY = (cardWidth + targetExtraHeight) / gradientWidth + 0.02
